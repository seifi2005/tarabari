...
def _add_code_block(doc, text: str) -> None:
    """
    Adds a pseudo code-block using a monospaced font.
    (python-docx doesn't have native fenced blocks.)
    """
    p = doc.add_paragraph()
    run = p.add_run(text)
...
def main() -> None:
    ...
    title = doc.add_paragraph("پرامپت‌های مرحله‌ای اجرای CMS خبری امن (Laravel 11)")
    title.runs[0].bold = True
    title.runs[0].font.size = Pt(18)

    doc.add_paragraph(
        f"تاریخ تولید: {today}\n"
        "هدف: ایجاد CMS خبری با Laravel 11 + Bootstrap 5 + MySQL با رعایت الزامات امنیتی افتا."
    )

    doc.add_paragraph("راهنما: هر مرحله را کامل کن، سپس پرامپت مرحله بعد را اجرا کن.")
...
ساختار فایل‌های view و کد Blade را مرحله‌ای بده.""",
    ...
    for heading, todo, prompt in stages:
        doc.add_paragraph("")
        h = doc.add_paragraph(heading)
        h.runs[0].bold = True
        h.runs[0].font.size = Pt(14)
        doc.add_paragraph(todo)
        doc.add_paragraph("متن پرامپت:")
        _add_code_block(doc, prompt)

    out_path = "/workspace/doc/CMS_Security_Prompts_Laravel11_FA.docx"
    doc.save(out_path)
    print(out_path)
